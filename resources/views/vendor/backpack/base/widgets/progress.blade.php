...
@includeWhen(!empty($widget['wrapper']), 'backpack::widgets.inc.wrapper_start')
  <div class="{{ $widget['class'] ?? 'card text-white bg-primary' }}">
    ...
    <div class="card-footer px-3 py-2">
      <a class="btn-block text-muted d-flex justify-content-between align-items-center " href="{{ $widget['footer_link'] ?? '#' }}"><span class="small font-weight-bold text-white">{{ $widget['footer_text'] ?? 'View more' }}</span><i class="la la-angle-right text-white"></i></a>
    </div>
    @endif
  </div>
